...
2. **Kiểm tra Pushgateway:**
   ```bash
   curl http://13.220.101.54:9091/metrics | grep github_workflow_run_total
   ```
...
3. **Kiểm tra Prometheus:**
   ```bash
   curl 'http://13.220.101.54:9090/api/v1/query?query=github_workflow_run_total'
   ```

4. **Kiểm tra Grafana Dashboard:**
   - Mở http://13.220.101.54:3030
